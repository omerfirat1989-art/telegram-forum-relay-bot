...
async def create_topic(user_id: int, user_name: str) -> int:
...
async def ensure_topic(user_id: int, user_name: str) -> int:
    topic_id = db_get_topic(user_id)
    if topic_id:
        return topic_id
    return await create_topic(user_id, user_name)

async def copy_user_message_to_panel(message: Message):
    user_id = message.from_user.id
    user_name = (message.from_user.full_name or "Kullanıcı").strip()

    topic_id = await ensure_topic(user_id, user_name)
    ...
    try:
        ...
        # Topic silindiyse / thread yoksa -> mapping temizle, yeni topic aç, tekrar dene
        if "message thread not found" in str(e).lower():
            logging.warning(f"message thread not found -> recreate topic for user_id={user_id}")
            db_clear_user(user_id)
            topic_id = await create_topic(user_id, user_name)
            await bot.copy_message(
                chat_id=FORUM_CHAT_ID,
                message_thread_id=topic_id,
                from_chat_id=message.chat.id,
...
async def start_handler(message: Message):
    ...
    await copy_user_message_to_panel(message)
...
async def admin_from_forum(message: Message):
    # sadece admin + sadece panel grubu
    if message.from_user.id != ADMIN_ID:
        return
